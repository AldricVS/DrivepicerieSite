...
require_once("includes/bdd.php");
    ...
    if(!isset($_POST["panierJson"]) || !isset($_POST["date"]) || !isset($_SESSION["utilisateur"])){
        ...
        //récupère le nombre de produits au total
        $nombreProduitsTotal = 0;
        for ($i=0; $i < count($panier); $i++) { 
            $nombreProduitsTotal += intval($panier[$i]->quantiteProduit);
        }
        ...
        if(!$result){
            ...
            for ($i=0; $i < count($panier); $i++) { 
                ...
                $quantiteResult = pg_query_params($bdd, 
                ...
                if($quantiteEnStock < $quantiteProduit){
                    $erreurs .= "Il n'y a que " . $quantiteEnStock . " de " . $nomProduit . " en stock. Commande annulée.\n";
                    
                    //on supprime d'abord les produit_commande car il est impossible de supprimer commande avant (relation par clé étrangère oblige)
                    pg_query($bdd, "DELETE FROM produit_commande WHERE id_commande=" . $idCommande);
                    pg_query($bdd, "DELETE FROM commande WHERE id_commande=" . $idCommande);
